...
L<POE::Component::Server::IRC|POE::Component::Server::IRC> suite. There are
included functions for uppercase and lowercase nicknames/channelnames and for
parsing mode lines and ban masks.
...
with C<parse_ban_mask()> to ensure that it is normalised) and something to
match against the IRC mask, such as a nick!user@hostname string. Returns 1 if
they match, undef/empty list otherwise. Returns undef if parameters are
missing. Optionally, one may pass the casemapping (see C<u_irc()>), as this
...
Takes two arguments, being a strings representing a set of IRC user modes
